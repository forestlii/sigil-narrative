...
内置：**`QuestStateCondition`**——当某任务处于所选状态时通过（`InProgress` / `Succeeded` / `Failed` /
`Finished` / `StartedOrFinished`）。从未开始的任务对所有查询都为 `false`。配合 `Not` 可表达“除非…”。

```csharp
// 例：仅当序章任务已成功后才提供此对话选项
...
内置：**`BeginQuestEvent`**——在宿主上开始一个任务（最常见的“和 NPC 对话即开任务”）。因开任务是一次性
副作用，默认 `RefireOnLoad = false`。

```csharp
var line = new DialogueNode_NPC("giveQuest", new DialogueLine("去把宝石带来。"));
line.AddEvent(new BeginQuestEvent(gemstoneQuest)); // 这行播放时开始任务
```
...
**`QuestBranch`** 同样是 `NarrativeNodeBase`，所以分支也带 `Events`。分支的事件在它**激活**时触发
（阶段 `Start`——所属状态被进入时）、**停用**时触发（阶段 `End`——分支被取用，或兄弟分支被取用而离开该状态）。
读档同样按 `RefireOnLoad` 过滤。取分支时其 End 事件**恰好触发一次**（停用是幂等的）。和状态一样，分支的
`Conditions` 继承但休眠——分支取用仍由任务驱动。
...
branch.AddEvent(new GrantRewardEvent(gold: 50) { Runtime = EEventRuntime.End }); // 此分支被取用时
```
