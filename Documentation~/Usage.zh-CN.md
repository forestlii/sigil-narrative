...
> **状态：`0.x`，开发中。** 存档/读档与图编辑器尚未发布（见[尚未包含的部分](#尚未包含的部分)）。
> 公开 API 未稳定，可能变动。
...
6. [对话](#对话)
7. [任务](#任务)
8. [`NarrativeComponent` 速查表](#narrativecomponent-速查表)
9. [从 Narrative Pro / UE 迁移过来？](#从-narrative-pro--ue-迁移过来)
10. [尚未包含的部分](#尚未包含的部分)
...
- **存档 / 读档。** 进度存在运行实例里（`MasterTaskList` 次数、任务的 `CurrentState` + 各任务项进度）。
  带 `bRefireOnLoad` 语义的 JSON DTO 存档层是下一个里程碑。`MasterTaskList.RestoreEntry` 与
  `QuestTask.RestoreProgress` 已作为读档抓手存在。
